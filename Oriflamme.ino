...
int relayBankSize = 8;
int delayMultiple = 200; 
int delayMultipleAdjust = 1;
int delayMultipleMIN = 25;
int delayMultipleMAX = 200;
...
long holdTimeout = 80;// amount of ms before we care about it being off or not. aka minum burst

//String sequence = "$#-1#-2#-3#-4#-5!4*";
String sequence = "";
...
void initializeRelays() {
  for (int i = 1; i <= relayBankSize; i++) {
    ...
    digitalWrite(getRelayPos(i), HIGH);// HIGH IS OFF...
  }
}
void arm() {
  digitalWrite(getRelayPos(relayBankSize), LOW); //Turn on light indicating the system is armed
...
void closeAllSelenoids() {
  for (int i = 1; i <= relayBankSize - 1; i++) {
    ...
    if (selenoidStates[i] == ON && !HOLDING) 
    {
...
//  ////Serial.println(mapHexToSelenoid(results.value));
  ...
  if (results.value == 0x9CB47) {
   ...
   //toggleRelay(3, ON);
   delayMultiple = delayMultiple + delayMultipleAdjust;
    
    if(delayMultiple > delayMultipleMAX)
    {
        delayMultiple = delayMultipleMAX;
    }
  }
  ...
  if (results.value == 0x5CB47) {
    ...
    if(delayMultiple < delayMultipleMIN)
    {
        delayMultiple = delayMultipleMIN;
    }
...
void loop() {  
  if (true){//irrecv.decode(&results)) { // THE BURSTS
   /* sequence =  "$1#-2#-3#-4#-5#6#7!2*"
                 "$-1#2#-3#-4#5#-6#7!1*"
                 "$1#-2#3#-4#-5#6#7!1*"
                 "$-1#-2#-3#4#5#6#-7!3*"
                 "$1#-2#-3#-4#-5#6#7!1*"
